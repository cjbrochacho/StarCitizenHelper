import json, os, time, threading, queue, ctypes
...
class App(tk.Tk):
 def __init__(s):
  super().__init__(); s.title('StarCitizenHelperv1.1'); s.geometry('980x760'); s.minsize(860,630); s.configure(bg='#101722'); s.protocol('WM_DELETE_WINDOW',s.close)
  s.cfg=D.copy();
  s.running_macro=''
  try:
  ...
  s.q=queue.Queue();s.stop=threading.Event();s.handles=[];s.game_running=False;s.game_foreground=False;s.game_check=0;s.guard_last_log=0;s.keep=s.scan=s.hold=False;s.held=[];s.hold_pending=False;s.hold_token=0;s.injected_until=0;s.last=time.monotonic();s.nextk=s.nexts=0;s.ignore=0;s.ui();s.register()
  ...
  keyboard.on_press(s.on_key_press) # keyboard module does not have on_click
  s.ml=None
  if mouse:
 ...
 def ui(s):
  ...
  h=tk.Frame(s,bg='#101722');h.pack(fill='x',padx=22,pady=(18,5));tk.Label(h,text='STAR CITIZEN HELPER v1.0',bg='#101722',fg='#eef6ff',font=('Segoe UI Semibold',18)).pack(anchor='w');tk.Label(h,text='Automation status and hotkey controls',bg='#101722',fg='#91a7bd').pack(anchor='w')
  ...
  s.guard=tk.Label(p,text='Alt+F4 protection: checking for StarCitizen.exe…',bg='#192433',fg='#9ebee0',font=('Segoe UI Semibold',10));s.guard.pack(anchor='w',padx=14,pady=(0,10))
