...
class App(tk.Tk):
 def __init__(s):
  super().__init__(); s.title('StarCitizenHelperv1.0'); s.geometry('980x760'); s.minsize(860,630); s.configure(bg='#101722'); s.protocol('WM_DELETE_WINDOW',s.close)
  s.cfg=D.copy();
  s.running_macro=''
  try:
  ...
  s.q=queue.Queue();s.stop=threading.Event();s.handles=[];s.game_running=False;s.game_foreground=False;s.game_check=0;s.guard_last_log=0;s.keep=s.scan=s.hold=False;s.held=[];s.hold_pending=False;s.hold_token=0;s.last=time.monotonic();s.nextk=s.nexts=0;s.ignore=0;s.ui();s.register()
